...
void Lasso::yank() {
    ...
    if (the_speedup != NULL) {
        the_speedup->resetCoin();
        the_speedup = NULL;
    }
    if (the_must_catch != NULL) {
        the_must_catch->resetCoin();
        the_must_catch = NULL;
    }
    if(the_magnet != NULL) {
...
void Lasso::setSpeed() {
    release_speed = INIT_RELEASE_SPEED;
    bool paused = true, rtheta = true;
    reset_all(lasso_start_x, lasso_start_y, release_speed, release_angle_deg, lasso_ax, lasso_ay, paused, rtheta);
}
...
void Lasso::check_for_speedup(SpeedupCoin *coinPtr) {
    double lasso_x = getXPos();
    double lasso_y = getYPos();
    double coin_x = coinPtr->getXPos();
    double coin_y = coinPtr->getYPos();
    ...
    if(distance <= LASSO_RADIUS) {
        the_speedup = coinPtr;
        the_speedup->getAttachedTo(this);
        the_speedup->allowSpeedup();
    }
} // End Lasso::check_for_speedup()

void Lasso::check_for_must_catch(MustCatchCoin *coinPtr) {
    double lasso_x = getXPos();
    double lasso_y = getYPos();
    double coin_x = coinPtr->getXPos();
    double coin_y = coinPtr->getYPos();
    ...
    double distance = sqrt((xdiff*xdiff)+(ydiff*ydiff));
    if(distance <= LASSO_RADIUS) {
        the_must_catch = coinPtr;
        the_must_catch->getAttachedTo(this);
    }
} // End Lasso::check_for_must_catch()
